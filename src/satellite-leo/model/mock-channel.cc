...
Ptr<NetDevice>
MockChannel::GetDevice (uint32_t i) const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_link[i];
}
      ...
      if (pLoss != 0)
    	{
          ...
          NS_LOG_DEBUG("src:"<< srcMob->GetPosition().GetLength());
          NS_LOG_DEBUG("dst:" << dstMob->GetPosition().GetLength());
      	  rxPower = pLoss->CalcRxPower (txPower, srcMob, dstMob);

      	  if (rxPower < -900.0)
    	    {
              NS_LOG_WARN("rxPower: " << rxPower);
