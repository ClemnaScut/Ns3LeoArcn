...
ARCN::SetPosition()
{
    ...
    {
        NS_LOG_DEBUG("node-" << i << " position:" << uanNode.Get(i)->GetObject<MobilityModel>()->GetPosition());
    }
}
...
ARCN::InstallInternetStack()
{
    ...
    select.SetBuoySatelliteMode(false);
    select.SetUanMaxId(uanNode.GetN()-1);
    // select.SetBuoyID(std::vector<uint32_t>({19}));
...
ARCN::NodeSendPacket()
{
    ...
    Ptr<Node> srcsend = uanNode.Get(m_uniformRandomVariable->GetValue(0, uanNode.GetN()-1));
    Ptr<Socket> srcsocket =  m_Uansockets[srcsend];
    Ipv4Address destAddr = uan_inter.GetAddress(m_uniformRandomVariable->GetValue(0, uanNode.GetN()-1),0);
    ...
    if(Simulator::Now().GetSeconds() < 20000)
    {
    Simulator::Schedule(nextSend, &ARCN::NodeSendPacket, this);
        NS_LOG_DEBUG("next Packet will be sent after " << nextSend.GetSeconds() << "s.");
    }
    ...
    // Ptr<Socket> srcsocket =  m_Uansockets[srcsend];cd
...
//	Throughput = double(m_SinkRecvNum * m_pktSize * 8)/ totalTime;
	...
	NS_LOG_UNCOND( "Time:" << int(Simulator::Now().GetSeconds())
    <<" DeliveryRatio:"<<std::setprecision(4)<<DeliveryRatio
