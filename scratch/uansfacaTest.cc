...
ARCN::Run() 
{
    ...
    SetupSocket();
    Simulator::Schedule(Seconds(10), &ARCN::NodeSendPacket, this);
    ...
    Simulator::Schedule(Seconds(300), &ARCN::Calculate, this);
...
ARCN::SetPosition()
{
    ...
    nodesPositionAlloc->Add (Vector(x*1, 0, 0));
    // nodesPositionAlloc->Add (Vector(x*2, 0, 0));
    nodesPositionAlloc->Add (Vector(x*100, x*100, 0));
...
//	aodv.Set("NetDiameter", UintegerValue(35));
	...
	if (printRoutes)
	{
		Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper>(
				"uanSFAMA", std::ios::out);
		aodv.PrintRoutingTableAllAt(Seconds(800), routingStream);
	}
}
