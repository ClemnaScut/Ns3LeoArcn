...
#ifndef VBF_ID_CACHE_H
#define VBF_ID_CACHE_H
...
public:
   ...
   * \param lifetime the lifetime for added entries
   */
	SendIdCache (Time lifetime) : m_lifetime (lifetime)
  {
  }
  /**
   * Check that entry (addr, id) exists in cache. Add entry, if it doesn't exist.
   ...
   * \returns true if the pair exists
   */
  bool IsSendDuplicate (Ipv4Address addr, uint32_t id);
  /// Remove all expired entries
  void Purge ();
  /**
   * \returns number of entries in cache
...
private:
  ...
  struct IsExpired
  {
     ...
     * \return true if expired, false otherwise
     */
    bool operator() (const struct UanUniqueId & u) const
    {
      return (u.m_expire < Simulator::Now ());
    }
  };
  /// Already seen IDs
  std::vector<UanUniqueId> m_SendidCache;
  /// Default lifetime for ID records
  Time m_lifetime;
};

class VBFRecvIdCache
{
public:
  /**
   * constructor
   * \param lifetime the lifetime for added entries
   */
	VBFRecvIdCache (Time lifetime) : m_lifetime (lifetime)
  {
  }
  /**
   * Check that entry (addr, id) exists in cache. Add entry, if it doesn't exist.
   ...
   * \returns true if the pair exists
   */
  bool IsRecvDuplicate (Ipv4Address addr, uint32_t id);
  /// Remove all expired entries
  void Purge ();
  /**
   * \returns number of entries in cache
...
private:
  ...
  /// Already seen IDs
  std::vector<UanUniqueId> m_RecvidCache;
  /// Default lifetime for ID records
  Time m_lifetime;
};
